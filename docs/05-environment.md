...
更新日期：2026-09-23 23:39 CST。依赖、PostgreSQL、API、基础集成检查和小程序构建已在当前 macOS 主机实跑。媒体/客户/员工扩展代码已构建，最新数据库迁移和扩展集成脚本待在可访问本机数据库的终端复验。
